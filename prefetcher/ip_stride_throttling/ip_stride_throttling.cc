...
constexpr double PREFETCH_ACCURACY = 0.2;
int prefetchdegree ;
double curr_prefetch_accuracy = 0;
...
void CACHE::prefetcher_initialize() { 
  std::cout << NAME << " IP-based stride throttling prefetcher" << std::endl; 
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  if (found != set_end) {
    ...
    if (stride != 0 && stride == found->last_stride){
      lookahead[this] = {addr, stride, prefetchdegree};
    }
  } else {
    // replace by LRU
...
{
  // std::cout << "Value of metadata_in: " << metadata_in << std::endl;
    if(pf_issued != 0){
    curr_prefetch_accuracy = (double)pf_useful/pf_issued;

    if(curr_prefetch_accuracy < PREFETCH_ACCURACY && prefetchdegree > 1){
      prefetchdegree--;
      // std::cout << "prefetchdegree: " << prefetchdegree << std::endl;
    }
    else if(curr_prefetch_accuracy > PREFETCH_ACCURACY){
      prefetchdegree++;
    }
  }
  return metadata_in;
...
{
  std::cout << " curr_prefetch_accuracy : " << curr_prefetch_accuracy << std::endl;
  std::cout << "prefetchdegree: " << prefetchdegree << std::endl;
}
