...
int PREFETCH_DEGREE = 2;  //possible values = 1,2,3
int PREFETCH_DISTANCE = 32;  //possible values = 16, 32, 64
int pf_configs[2][6]; // stores possible (degree, distance) pairs
float A_LOW_THR = 0.025;
float A_HIGH_THR = 0.06;
float ACCURACY = 0;
int DYN_CONFIG_COUNTER = 3; // takes values 0 to 6, initialized to a middle ground
...
void CACHE::prefetcher_cycle_operate()
{
  ...
  if (auto [old_pf_address,degree,direction] = lookahead[this]; degree > 0) {
   //cout<<"old_pf_addr = "<<old_pf_address<<" degree "<<degree<<" direction "<<direction<<endl;
    uint64_t pf_address = 0;
    if(direction==1)  pf_address  = old_pf_address + (1 << LOG2_BLOCK_SIZE);
    else pf_address = old_pf_address - (1 << LOG2_BLOCK_SIZE);
    ...
    if (virtual_prefetch || (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {
      ...
      if (success)
      {
        ...
        //cout<<"pf_addr = "<<pf_address<<" degree "<<degree<<endl;
...
//if 
  //cout << streams.begin()->second[0].state;
    for(int i=0;i<N_STREAMS;i++)
    {
      ...
      //cout << "cl_addr " << cl_addr<< " Stream no " << i << " start ptr " << stream.start_pointer << " end ptr " << stream.end_pointer << " state " << stream.state << " direction " << stream.direction <<endl;
      ...
      if(streams.begin()->second[N_STREAMS-1].state!=0 && stream_detected==0)
        {
          ...
          streams.begin()->second[lru_stream] =  {cl_addr,0,0,0} ;
        }
...
{
  if(pf_issued==0) ACCURACY = 0;
  else
  {
    ACCURACY = float(pf_useful)/float(pf_issued);
    // cout<< pf_useful << " " << pf_issued << endl;
    // cout<<"Acc "<< ACCURACY << endl;
    if(ACCURACY>A_HIGH_THR)
    {
      if(DYN_CONFIG_COUNTER<6)
      {
        DYN_CONFIG_COUNTER++;
        PREFETCH_DEGREE = pf_configs[0][DYN_CONFIG_COUNTER];
        PREFETCH_DEGREE = pf_configs[1][DYN_CONFIG_COUNTER];
        // cout<<"aggressiveness inc"<<endl;
      }
    }
    else if(ACCURACY<A_LOW_THR)
    {
      if(DYN_CONFIG_COUNTER>0)
      {
        DYN_CONFIG_COUNTER--;
        PREFETCH_DEGREE = pf_configs[0][DYN_CONFIG_COUNTER];
        PREFETCH_DEGREE = pf_configs[1][DYN_CONFIG_COUNTER];
        // cout<<"aggressiveness dec"<<endl;
      }
    }
  }
  return metadata_in;
